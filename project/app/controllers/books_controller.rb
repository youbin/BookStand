class BooksController < ApplicationController
  ...
  def attach_book
    ...
    get_data_path = 'lib/assets/book_add.py ' + isbn
  ...
  # POST /books.json
  def create
    @book = Book.new(:b_id => params[:isbn], :b_title => params[:title], :b_thumb => 0, :b_totalStar => 0, :b_starNum => 0, :b_likeCount => 0, :b_belongCount => 0)
    @book.b_count = 0
    @book.b_likeCount = 0
    book_id = @book._id
  ...
  def detail
    ...
    @book_detail = BookDetail.find(book_isbn)

    #reviews = BookReview.find(params[:isbn]).b_reviews
    #@book_review = Review.find()
    #@book_review = reviews
    render json: {"book_detail" => @book_detail}, status: :accepted
  end
